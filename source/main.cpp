...
{
    google::InitGoogleLogging(argv[0]);//使用glog之前必须先初始化库，仅需执�?一次，�?��内为程序�?
    FLAGS_alsologtostderr = true;     //�?��将日志输出到文件和stderr
    FLAGS_colorlogtostderr = true;    //�?���?��不同颜色显示
    FLAGS_log_dir = "../logs"; //
    // LOG(INFO) << "info";
    // LOG(WARNING) << "warning";
    // LOG(ERROR) << "error";
    ...
    google::ShutdownGoogleLogging();  //当�?结束glog时必须关�?��，否则会内存溢出
    return 0;
}